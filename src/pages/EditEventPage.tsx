...
const EditEventPage: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (
      ...
      !eventData?.startDate ||
      !eventData?.location ||
      !(eventData?.categories && eventData.categories.length)
    ) {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/events/${eventData.id}`, {
        method: 'PUT',
      ...
      // Update local context for immediate UI sync
      if (typeof updateEvent === 'function') {
        updateEvent(updated as Event);
      }
